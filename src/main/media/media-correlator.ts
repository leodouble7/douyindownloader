...
function contentId(url: string): string | undefined { const parsed = new URL(url); return parsed.searchParams.get('media_id') ?? parsed.searchParams.get('content_id') ?? parsed.searchParams.get('asset_id') ?? (/^(?:[\w-]+\.)*douyinvod\.com$/i.test(parsed.hostname) ? parsed.searchParams.get('video_id') : undefined) ?? undefined; }
