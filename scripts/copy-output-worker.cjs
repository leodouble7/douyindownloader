...
function copyOutputWorkers() {
  const destination = join(__dirname, '../out/main');
  mkdirSync(destination, { recursive: true });
  for (const file of ['output-worker.cjs', 'windows-output.cjs', 'output-names.cjs']) copyFileSync(join(__dirname, '../src/main/download', file), join(destination, file));
...
module.exports = copyOutputWorkers;
if (require.main === module) copyOutputWorkers();
